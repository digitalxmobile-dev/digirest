    ...
    ConnectionService.getConnection(
        function onConnected(error,dbConnection){
            if(error){
                ...
                var key = _logQueryStart(collection,pipeline);
                _collection.aggregate(pipeline,
                    options,
                    function onAggregate(error,array){
                        try {
                    ...
                    function onGet(err,obj){
                        ...
                        try {
                            if(collection!=ERROR_COLLECTION) {
                                console.log(MODULE_NAME + ': Insert Operation %s', JSON.stringify(obj));
                            }
                        }catch(err){
                            // ignore
                        }
...
function _logQueryStart(collection,query){
    ...
    try {
        if (process.env.PRINT_QUERY) {
            if(collection!=ERROR_COLLECTION) {
                var key = MODULE_NAME + ': [' + collection + '].' + JSON.stringify(query);
                console.log(MODULE_NAME + ': running query coded %s {[%s]}', timingKey, '[' + collection + '].' + JSON.stringify(query));
            }
            console.time(timingKey);
        }
    }catch(err){
        console.log(MODULE_NAME + ': cannot track query due tue error %s', err.message);
        err = null;
